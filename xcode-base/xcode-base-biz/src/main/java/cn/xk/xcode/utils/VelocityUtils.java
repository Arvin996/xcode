...
public class VelocityUtils {
     ...
     * @return 模板列表
     */
    public static VelocityContext prepareContext(GenTableResultVo genTable) {
        ...
        velocityContext.put("datetime", LocalDateTimeUtil.formatNormal(LocalDateTime.now()));
    ...
    public static void setMenuVelocityContext(VelocityContext context, GenTableResultVo genTable) {
    ...
    public static void setTreeVelocityContext(VelocityContext context, GenTableResultVo genTable) {
        String options = genTable.getOptions();
        JSONObject paramsObj = JSON.parseObject(options);
    ...
    public static void setSubVelocityContext(VelocityContext context, GenTableResultVo genTable) {
        GenTableResultVo subTable = genTable.getSubTable();
        String subTableName = genTable.getSubTableName();
        String subTableFkName = genTable.getSubTableFkName();
        String subClassName = genTable.getSubTable().getClassName();
        String subTableFkClassName = convertToCamelCase(subTableFkName);
     ...
     * 获取文件名
     */
    public static String getFileName(String template, GenTableResultVo genTable) {
        // 文件名称
        String fileName = "";
        // 包路径
        String packageName = genTable.getPackageName();
     ...
     * @return 返回需要导入的包列表
     */
    public static HashSet<String> getImportList(GenTableResultVo genTable) {
        List<GenTableColumnPo> columns = genTable.getColumns();
        GenTableResultVo subGenTable = genTable.getSubTable();
        HashSet<String> importList = new HashSet<String>();
        if (Objects.nonNull(subGenTable)) {
            importList.add("java.util.List");
        }
        for (GenTableColumnPo column : columns) {
     ...
     * @return 返回字典组
     */
    public static String getDicts(GenTableResultVo genTable) {
        List<GenTableColumnPo> columns = genTable.getColumns();
        Set<String> dicts = new HashSet<String>();
        addDicts(dicts, columns);
        if (Objects.nonNull(genTable.getSubTable())) {
            List<GenTableColumnPo> subColumns = genTable.getSubTable().getColumns();
     ...
     * @param columns 列集合
     */
    public static void addDicts(Set<String> dicts, List<GenTableColumnPo> columns) {
        for (GenTableColumnPo column : columns) {
     ...
     * @return 展开按钮列序号
     */
    public static int getExpandColumn(GenTableResultVo genTable) {
        String options = genTable.getOptions();
        JSONObject paramsObj = JSON.parseObject(options);
        String treeName = paramsObj.getString(GenConstants.TREE_NAME);
        int num = 0;
        for (GenTableColumnPo column : genTable.getColumns()) {
