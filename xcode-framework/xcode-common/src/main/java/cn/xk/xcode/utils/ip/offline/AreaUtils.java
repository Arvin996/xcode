package cn.xk.xcode.utils.ip.offline;

import cn.hutool.core.io.resource.ResourceUtil;
import cn.hutool.core.lang.Assert;
import cn.hutool.core.text.csv.CsvRow;
import cn.hutool.core.text.csv.CsvUtil;
import cn.xk.xcode.pojo.Area;
import cn.xk.xcode.pojo.AreaTypeEnum;
...
import static cn.xk.xcode.utils.collections.CollectionUtil.findFirst;
...
public class AreaUtils {
    ...
    public static String format(Integer id, String separator) {
        ...
        for (int i = 0; i < AreaTypeEnum.values().length; i++) { // 避免死循环
            ...
            if (area == null
                    || ObjectUtil.equalsAny(area.getId(), Area.ID_GLOBAL, Area.ID_CHINA)) { // 跳过父节点为中国的情况
                break;
            }
    ...
    public static Integer getParentIdByType(Integer id, @NonNull AreaTypeEnum type) {
        for (int i = 0; i < Byte.MAX_VALUE; i++) {
            ...
            // 情况二：找到根节点，返回空
            if (area.getParentArea() == null || area.getParentArea().getId() == null) {
                return null;
            }
            // 其它：继续向上查找
            id = area.getParentArea().getId();
        }
        return null;
    }
