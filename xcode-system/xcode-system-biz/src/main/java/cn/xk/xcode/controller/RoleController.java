...
@Tag(name = "角色管理")
@RestController("/role")
public class RoleController implements SystemRoleClient {
    ...
    public CommonResult<List<UserPo>> queryUsersByRole(@RequestBody UpdateRoleDto updateRoleDto)
    {
        ...
        List<UserPo> userPos = userMapper.selectListByIds(list.stream().mapToLong(UserRolePo::getUserId).boxed().collect(Collectors.toList()));
    ...
    @Override
    public CommonResult<SystemRoleResultVo> getRole(Integer id) {
        RolePo rolePo = roleService.getById(id);
        if (rolePo != null){
            return CommonResult.success(new SystemRoleResultVo().setId(rolePo.getId()).setName(rolePo.getName()));
        }
