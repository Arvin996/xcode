...
@Tag(name = "用户管理")
@RequestMapping("/user")
public class UserController implements SystemUserClient {
    @Resource
    ...
    @Override
    @GetMapping("/getUser")
    public CommonResult<SystemUserResultVo> getUser(@RequestParam("id") Long id) {
        return CommonResult.success(userService.getUser(id));
    }

    @Override
    @GetMapping("/selectNotUserList")
    public CommonResult<List<SystemUserResultVo>> selectNotUserList(@RequestParam("userIds") Collection<Long> userIds) {
        List<UserPo> list = userService.list(USER_PO.ID.notIn(userIds));
        if (CollectionUtil.isNotEmpty(list)) {
            return CommonResult.success(BeanUtil.toBean(list, SystemUserResultVo.class));
        }
    ...
    public CommonResult<List<SystemUserResultVo>> selectUserList(@RequestParam("userIds") Collection<Long> userIds) {
        List<UserPo> list = userService.list(USER_PO.ID.in(userIds));
        if (CollectionUtil.isNotEmpty(list)) {
            return CommonResult.success(BeanUtil.toBean(list, SystemUserResultVo.class));
        }
        return CommonResult.success(Collections.emptyList());
